...
vec3 DirectLighting(vec3 lightPosition, vec3 lightColor, vec3 normal, vec3 dirToCam, float metalness, float roughness, vec3 albedo)
{
    vec3 dirToLight = normalize(lightPosition - inPosition);
    vec3 halfway = normalize(dirToLight + dirToCam);

    float dist = length(lightPosition - inPosition);
...
void main()
{
    ...
    for (int i = 0; i < 4; i++)
    {
        direct += DirectLighting(scene.lightPoint[i].position, scene.lightPoint[i].color, normal, dirToCam, metal, rough, albedo);
    }
